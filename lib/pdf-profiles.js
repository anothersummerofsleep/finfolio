...
// latest 4-digit year anywhere in the document": a statement can carry
// forward-looking tables (e.g. an installment plan's future expiry date,
// months or years out) whose year is not the statement's own — scanning the
// whole document for the max year picks that up and mis-dates every
// year-less row on the statement. When no period line is found at all, fall
// back to a flat max-year scan with no month attached (old behaviour).
...
export const PROFILES = [hsbc, trust, generic];
