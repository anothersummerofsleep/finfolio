...
export function inferPeriod(rawText) {
...
export function resolveDate(token, period) {
...
function parseDbsBank(pages) {
  ...
  const transactions = [];
  const errors = [];
  // Opening/closing balance per cash account, read off the Brought Forward /
  // Carried Forward rows the transaction loop skips. Account NAME only — the
  // account number stays out of everything finfolio stores or displays.
  const balances = [];
  let bal = null;
  let inCash = false;
  let cols = null;
  ...
  for (const line of lines) {
    ...
    if (/account\s*no\.?/i.test(line.text)) {
      ...
      bal = null;
      if (inCash) {
        // The header repeats when an account's table continues onto the next
        // page — same account, same record (so opening stays the statement's
        // true opening and closing ends up the final Carried Forward figure).
        const account = line.text.replace(/\s*account\s*no\.?.*$/i, '').trim();
        bal = balances.find((b) => b.account === account);
        if (!bal) {
          bal = { account, opening: null, closing: null };
          balances.push(bal);
        }
      }
      last = null;
    ...
    if (!inCash) continue;
    if (SKIP_ROW.test(line.text)) {
      // Not transactions, but the Brought/Carried Forward rows carry the
      // account's balance — the last number on the line ("Total Balance
      // Carried Forward in SGD: <withdrawals> <deposits> <balance>"). Opening
      // keeps the first hit (a page break repeats Brought Forward mid-account);
      // closing keeps the last (the final Total row supersedes per-page ones).
      const nums = line.text.match(/\d[\d,]*\.\d{2}/g);
      if (bal && nums) {
        const value = parseAmount(nums[nums.length - 1]);
        if (value != null) {
          if (/brought\s+forward/i.test(line.text)) bal.opening = bal.opening ?? value;
          else if (/carried\s+forward/i.test(line.text)) bal.closing = value;
        }
      }
      last = null;
      continue;
  ...
  return {
    ...
    errors,
    meta: {
      year: period ? period.endYear : null,
      bank: 'dbs-bank',
      balances: balances.filter((b) => b.opening != null || b.closing != null)
    }
  };
}
